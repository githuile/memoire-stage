...
\begin{itemize}
\item[$\bullet$] \textbf{Luc Desruelle} Mon maître de stage qui m'a accueilli dans l'entreprise et m'a suivi tout au long de mon parcours. Il m'a permis de développer mes connaissances en aiguillant mes choix techniques
\item[] 

\item[$\bullet$] \textbf{Benjamin Lerch} Développeur dans l'équipe Mesulog mais aussi responsable du service informatique, il a été d'une grande aide pour appréhender le système d'information de l'entreprise et m'a porté conseil en cas de problème.
\item[] 

\item[$\bullet$] \textbf{Kenzo Sechi} Qui était aussi en alternance chez Mesulog, qui m'a aidé à m'intégrer dans l'équipe et a été d'une grande aide pour toute question.
\item[] 

\item[$\bullet$] \textbf{Toute l'équipe Mesulog} Pour l’accueil à bras ouvert, qui m'a aidé à m'intégrer dans l'équipe. Pour leur aide et leurs conseils au long de mon stage.
\item[]
...
%abstract
\section*{Abstract}
This report presents different missions of a 3 and a half internship.
This end-of-degree internship was located at the company Mesulog, in Moirans.
The main subject of this internship is system and network administration, with an emphasis on the cybersecurity stakes of these missions.
...
This report outlines the various stages of the process, the challenges encountered, and the solutions that were found by chronological order.
...
Nous avons ensuite \textbf{Damien Lamy} et \textbf{Adrien Andrieu}, chefs de projet. Ils ont aussi des compétences de développeurs et présentent un profil technico-commercial. Le reste de l'équipe est composée de développeurs, à noter que Benjamin Lerch est également le RSSI du service informatique, il est donc mon supérieur direct.\\
...
\textbf{Emilie Evrard} est quant à elle la responsable administrative de Nerys Group. Les activités de comptabilité et de ressources humaines étant mutualisées entre les entreprises du groupe. Certains acteurs extérieurs à l'équipe de Mesulog travaillent de concert avec l'équipe, notamment des salariés de Nerys et Nerys Groups Service. Cependant ils ne sont pas représentés sur ce schéma organisationnel car je n'ai aucun contact avec eux dans mes missions.\\
\\
Je suis donc entouré d'une équipe expérimentée qui peut m'apporter de l'aide et des conseils en cas de problème. J'ai aussi un contact direct avec mes supérieurs que ce soit Benjamin Lerch ou Luc Desruelle, mon maître de stage. Cela me permet de bénéficier d'un suivi régulier dans mes missions.
\newpage
...
L'objectif initial de ce stage était la mise en place d'outils de supervision et de management d'événements de sécurité, tels que Suricata et Wazuh.\\
Cependant ce n'est pas la première mission que j'ai effectuée, en effet pour appréhender ce nouvel environnement j'ai commencé par une mission d'analyse et de mise en forme de données sur une base de données Access.\\
...
J'ai aussi pu retravailler l'organisation de l'Active Directory de l'entreprise, avec des mises en place de GPO, ou encore participé à la création d'une campagne interne de test de phishing. Certaines plus petites tâches se sont intercalées entre ces sujets principaux, elles ne seront pas détaillées dans ce rapport.\\
\\
Ces missions étant toutes étroitement liées entre elles, excepté l'analyse de données pour un client, elles se sont déroulées en parallèle sur la durée de mon stage.
\par\bigskip
\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{%
...
\subsection{Base de données Access}
  ...
  Au début de mon stage, j'ai travaillé sur une analyse de base de données Microsoft Access pour un client de Mesulog. Cette base de données, générée automatiquement depuis les rapports de test de banc de test Mesulog présent chez des clients, se présente au départ comme 1 seule table à 23 colonnes.\\
  \begin{figure}[H]
    \centering
  ...
  \\
  Les modules sont testés sur un banc, ils sont soit de type 1 ou 2. Un module passe un ou plusieurs tests ou essais avant d'être validé. Pour qu'un module soit validé il suffit d'un essai passé. Un essai à 4 issues : "Passed" (valide), "Failed" (non-valide), "Terminated" (le test s'est terminé prématurément) et "Error" (le banc de test a fait crasher le test).\\
  \\
  Nous devons donc produire un tableau récapitulatif général par année et par banc avec entre autre les nombres de modules validés, le nombre d'essais moyen et médian avant pour un module avant d'être validé, le temps moyen en heure pour valider un module, le ratio des états des essais.\\
  \\
...
\subsubsection{Nettoyage des données}
  ...
  \begin{enumerate}
    ...
    \item[$\bullet$] Les essais qui font partie de la chaîne production et les essais de R\&D ou de SAV qui ne sont pas signalés de manière consistante. Il faut écarter ces enregistrements de l'analyse.
  \end{enumerate}
\par\bigskip
  Pour ce faire nous avons créé une suite de requêtes SQL pour automatiquement effectuer ce nettoyage. Étant donné que l'objectif final est d'automatiser l'exécution de toutes ces requêtes dans Microsoft Access avec un script VBS (Visual Basic Scripting), la syntaxe SQL utilisée est celle de Microsoft Access, qui nous impose des choix parfois sous-optimaux.\\
  Pour ne pas surcharger ce rapport nous verrons ici seulement un exemple de formatage, le reste est disponible en \hyperref[fig:setup-access]{Annexe} :
  \par\bigskip
  \begin{figure}[H]
...
Nous pouvons voir ci-dessus une requête destinée à formater tous les numéros de série sous la forme "*.XXX", "XXX" étant 3 chiffres.\\
Exemple : "17A240923.2" est transformé en "17A240923.002"\\
Afin de ne perdre aucune information, le numéro de série normalisé est inséré dans une nouvelle colonne. Il en va de même pour le modèle et pour l'objectif du test (Production ou SAV, R\&D).\\
\\
Une fois que toutes les données sont complétées et formatées, nous passons à l'exploitation de celles-ci.
\par\bigskip
...
\begin{figure}[H]
  ...
  \label{fig:tableau}
  \caption{Tableau récapitulatif général - (pour le banc VM-TD14-BHT)}
\end{figure}

Ce tableau récapitulatif permet de voir facilement l'évolution de l'utilisation du banc, des résultats d'essais et du nombre de modules sortis en production.\\
...
\\
Dans ce jeu de données, nous avons 2 bancs et 2 types de modules. Il était demandé d'analyser l'évolution de la production par banc tous types de modules confondus et par banc seulement pour les modules types V2. Cela nous fait donc 6 tableaux à remplir de données.\\
Pour extraire ces données, nous partons de la table nettoyée, et nous avons fait une suite de 18 requêtes SQL pour extraire les données ou créer des tables temporaires pour d'autres requêtes. Si dans ces requêtes nous définissons le banc et le type de module en dur, il est prévu d'utiliser un script VBS (Visual Basic Scripting) pour remplacer automatiquement ces variables dans les requêtes.
\newpage
Comme précédemment, nous ne montrons pas l'intégralité des requêtes dans le corps du rapport, mais elles sont disponibles en \hyperref[fig:extraction-access]{ici}.
\begin{figure}[H]
  ...
  \caption{Requête d'une table temporaire pour le calcul du TtFP (Time to First Pass)}
\end{figure}
\par\bigskip
Cette requête crée une table temporaire qui regroupe l'empreinte temporelle du premier essai d'un module, du premier essai validant, de la différence entre ces deux essais, du temps d'essai du second essai et finalement du "Time to First Pass" arrondi à l'heure.
\newpage
...
\end{figure}
Suricata peut donc décider de ne rien faire, de générer une alerte ou de rejeter la trame en mode IPS (Intrusion Prevention System). En mode IDS (Intrusion Detection System) nous lançons simplement des alertes mais sans bloquer le trafic. C'est comme ça que nous l'installerons, afin d'éviter qu'un faux positif coupe une connexion légitime.\\
 Pour installer Suricata, nous avons pris une machine physique. Pour éviter de risquer de ralentir le trafic, nous avons mis en place un port mirroring à partir d'un switch juste en aval du pare-feu Sophos, le Suricata reçoit donc une copie exacte de tout le trafic qui passe de WAN à LAN et de LAN à WAN.\\
 \\
 ...
 \newpage
  Cependant ce dépôt qui installe via un fichier rpm n'arrive pas à résoudre ses dépendances sur Rocky 10. Nous avons donc choisi d'installer Suricata via l'image Docker fournie par Jason Ish, membre du projet OISF.\\
  Ce choix technique nous permet aussi d'installer Evebox dans un second conteneur Docker. Evebox est l'outil de gestion d'alerte et d'événement pour Suricata développé et maintenu lui aussi par le projet OISF.
...
\end{figure}
\par\bigskip
Premièrement nous pouvons voir que les deux conteneurs sont paramétrés pour redémarrer automatiquement dès qu'ils sont éteints, pour limiter le temps hors-service. La variable d'environnement "TZ" permet de s'assurer que l'heure soit bien synchronisée dans les logs.\\
Quelques points importants :\\
  \begin{enumerate}
    ...
    \item[$\bullet$] La variable d'environnement "EVEBOX\_RETENTION\_PERIOD" permet de définir une limite de temps au-delà de laquelle les logs sont effacés, très important pour ne pas saturer le stockage. Ici la limite est de 7 jours.
  \end{enumerate}
  \par\bigskip

Ci-dessous l'interface web de Evebox, qui permet de suivre pas seulement les alertes mais tous les événements Suricata : \\
\begin{figure}[H]
    \centering
...
Pour éviter de surcharger le SIEM, nous avons créé un deuxième fichier de log sous le format EVE (Extensible Event Format), intitulé eve-wazuh.json, dans lequel seulement les alertes Suricata sont enregistrées, contrairement au fichier de log eve.json pour Evebox où toutes les alertes ainsi que tous les événements sont consignés.\\
...
\begin{figure}[H]
  ...
  \caption{Dashboard d'événement Docker dans Wazuh}
\end{figure}
\par\bigskip
En ce qui concerne les règles de Suricata en mode IDS, nous avons premièrement ajouté les règles de l'Open Emerging Threats publiées par Proofpoint. Avec des règles additionnelles, ajoutées à la suite de l'analyse de l'activité réseau avec Evebox.\\
Par exemple, en constatant un trafic significatif de paquets mDNS sur le réseau, des règles contre le mDNS poisoning ont été ajoutées dans un fichier de règle supplémentaire.\\
Afin de se prémunir contre les IP malicieuses, nous avons ajouté des règles automatiquement suivant une liste d'IP malicieuses.\\
Pour ce faire, nous avons écrit un programme bash qui récupère des listes d'IP malicieuses depuis des listes de emerging threats, abuse.ch ou encore alien vault (Voir annexe). Nous concaténons ensuite ces listes dans un seul fichier, puis ajoutons pour chaque IP 2 règles dans un fichier de règle Suricata "ip-reputation.rules"
une règle pour les connexions sortantes et la deuxième pour les connexions entrantes depuis/vers cette IP. Ensuite le programme exécute "docker kill --signal=USR2 suricata" qui envoie un signal non pas d'arrêt mais de rechargement des règles à chaud sans redémarrer le service.\\
Ce rechargement est d'ailleurs détecté par l'agent Wazuh présent sur la machine hôte, qui remonte un événement de rechargement dans Wazuh.\\
\newpage
\begin{figure}[H]
    \centering
...
\end{figure}
\par\bigskip
Pour exécuter ce programme de manière automatique, nous avons créé un service Linux "suricata-ip-update.service" qui est lancé tous les jours à minuit par "suricata-ip-update.timer".\\
Grâce à l'option "RandomizedDelaySec=1800", ce timer a une fenêtre de 30 min (entre 00:00 et 00:30) pour lancer le service, ce qui permet d'éviter de surcharger la machine si des processus lourds sont déjà en train de s'exécuter (sauvegardes automatiques entre autre).\\
\newpage
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
  ...
  \caption{Service Linux suricata-ip-update}
\end{figure}
\par\bigskip

\subsection{Installation et Intégration de Wazuh}

En parallèle de l'installation de Suricata, nous avons mis en place un SIEM (Outil de gestion d'événements et d'informations de sécurité). Nous avons opté pour le SIEM open source Wazuh.\\
...
Un SIEM marche donc main dans la main avec des systèmes IDS pour garantir une meilleure sécurité du système d'information. Le SIEM sert aussi de centralisateur et archiveur de logs, ce qui permet de remplir des critères ISO 27001.\\
...
Une fois les logs reçus par le serveur Wazuh, il faut analyser ces événements. Pour ce faire, Wazuh utilise un fonctionnement en 3 étapes :

\begin{enumerate}
  \item[$\bullet$] Premièrement, on utilise une fonction de prematch pour filtrer les logs à considérer et à ignorer.
  \item[$\bullet$] Ensuite, Wazuh utilise des \textbf{décodeurs}. Ces décodeurs extraient les champs importants des logs à l'aide de regex.
  \item[$\bullet$] Dernièrement, les champs extraits passent aux travers de \textbf{règles}. Ces règles définissent une ou plusieurs limites à ne pas dépasser, ainsi que le niveau de l'alerte à générer.
\end{enumerate}
...
Ce décodeur nommé "\textbf{ubiquiti-dhcp-ack}", utilise le prematch "\textbf{dnsmasq-dhcp}" : Il ne considère que les logs où la chaine de caractères "\textbf{dnsmasq-dhcp}" est présente. De la même manière, la chaîne "\textbf{DHCPACK}" est présente en texte brut afin de ne considérer seulement l'événement lorsque la connexion est établie et éviter de faux positifs.\\
\\
Afin de séparer les différents champs, nous utilisons une regex, les champs à extraire sont délimités par des parenthèses. Si des parenthèses sont présentes dans le log, elles pourront être matchées en échappant la parenthèse du regex avec un antislash.\\
...
\end{figure}
\par\bigskip
Une fois que les règles sont déclenchées, il ne reste plus qu'à créer des alertes afin de prévenir les administrateurs système et réseau par mail.
\newpage
\subsubsection{Wazuh API}

Wazuh possède aussi une API, accessible par le web, qui nous permet d'interagir avec le serveur Wazuh depuis d'autres applications. J'ai pu utiliser cette API pour externaliser certains traitements de logs incommodes sur Wazuh.\\
\\
L'objectif principal d'analyser les journaux de logs du pare-feu Sophos était de détecter une fuite de données vers un tiers. Ainsi comme l'exemple précédent nous avons créé plusieurs décodeurs pour extraire le nombre d'octets de chaque trame enregistrée par le pare-feu. Cependant, il s'est avéré très compliqué de correctement détecter une fuite de donnée :\\
\\
En effet, pour mesurer un transfert de donnée de 1Go sur 1 heure, nous pourrions faire une première règle de niveau 0 (qui ne génère pas d'alerte) avec une limite d'octets arbitraire de 10 Mo, puis une deuxième règle se déclenchant seulement si la première règle a été déclenchée 100 fois en une heure. La logique étant que 100 fois 10 Mo fait 1 Go.\\
Cependant, cette façon de faire (en utilisant les paramètres "frequency" et "timeframe" de Wazuh) n'est pas viable car il y a une infinité de multiplications possibles pour atteindre une limite de 1 Go. Dès lors, il est nécessaire d'externaliser ce traitement. Nous avons décidé d'utiliser un script Python pour automatiser ce processus.\\
\\
Ce script nommé "sophos-exfiltration.py" est présent sur la machine Wazuh dans "/var/ossec/integrations". Étant donné que ce script ne rentre pas sur une page entière, il est disponible en \hyperref[fig:sophos-integration]{Annexe}.\\
\\
Premièrement, nous récupérons un token d'authentification à l'API Wazuh avec les identifiants d'un utilisateur créé dans Wazuh pour ce programme. Ensuite, nous recherchons tous les événements produits entre l'heure actuelle et un délai de temps défini, ici 1h avant ; les arguments de cette requête sont encodés en JSON. Ensuite, nous regroupons ces données par adresse ip source et faisons la somme du nombre d'octets envoyés.\\
...
\end{figure}
\par\bigskip
Il est important de laisser le niveau de sévérité de cette règle à 0, car cette règle se déclenche si une alerte du même ID est active. L'alerte générée par le programme Python a déjà une sévérité qui primera sur une sévérité 0, mais la règle Wazuh pourrait se déclencher elle-même en boucle si elle émettait une alerte. Une règle au niveau 0 ne déclenche pas d'alerte, empêchant cette réaction en chaine.
\newpage
\subsection{Durcissement système et administration Windows Server}

L'installation de Wazuh et plus particulièrement des agents Wazuh sur les ordinateurs du parc informatique de Mesulog permet plus que de simplement remonter les journaux de logs. En effet, un agent Wazuh peut aussi vérifier en temps réel la configuration des équipements et donc la conformité à un benchmark de sécurité.

\subsubsection{CIS Benchmark}

Le standard de durcissement système de CIS (Center for Internet Security) est un standard de sécurité pour de nombreux systèmes d'exploitation qui s'aligne avec les exigences de la norme ISO 27001 car il permet de mesurer la gestion des accès, l'exploitation sécurisé et la protection des actifs. Tout le parc informatique (hors serveurs) est en Windows 11 entreprise. Ainsi nous nous concentrons ici sur le benchmark CIS pour Windows 11. Afin de réduire la surface d'attaques des postes, le benchmark CIS se base sur des GPOs (Group Policies Objects) à déployer sur le parc.\\
Les benchmarks CIS sont organisés en 2 niveaux :
  ...
  ce qui permet la détection et la validation automatisée des configurations de sécurité sans outils supplémentaires que Wazuh.

\par\bigskip
\subsubsection{Structure de l'Active Directory}

Toutes les règles et recommandations du benchmark CIS ne peuvent pas être appliquées à tous les postes sans distinction. En effet certaines activités métier de l'entreprise, comme le développement ; nécessitent des permissions plus élevées que pour de la simple bureautique. On pourrait citer par exemple l'exécution de programmes non-signés.\\
Or l'organisation de l'Active Directory initialement présente ne permettait pas cette granularité.
...
\begin{itemize}
  \item[$\bullet$] \textbf{Ordinateurs} composé de l'OU \textbf{Postes} répartis entre les \textbf{Administrateurs}, \textbf{Développeurs}, \textbf{Direction} et \textbf{Stagiaires} ainsi que de l'OU \textbf{Serveurs}
  \item[$\bullet$] \textbf{Utilisateurs} composé des différents utilisateurs du domaine également répartis entre les \textbf{Administrateurs}, \textbf{Développeurs}, \textbf{Direction} et \textbf{Stagiaires}
...
de développer dans des machines virtuelles, les autres fonctions de l'entreprise comme la Direction qui ne s'occupe que de documents légaux et organisationnels ne bénéficient pas de ces droits car inutiles dans leurs cas.
\newpage
...
Les GPOs de durcissement sont donc déployées sous forme itérative et incrémentielle. L'ensemble des GPOs
...
\end{itemize}
Et donc de définir leur application ou non, leur champ d'application (Unités d'Organisations affectées) et leur priorité (dans quelle itération cette GPO est à déployer).
\par\bigskip
Le déploiement a donc été réparti en plusieurs itérations successives d'une trentaine de règles. Ces itérations sont espacées dans le temps de plusieurs jours entre chaque, afin de détecter tout problème ou dysfonctionnement par rapport à un cas d'utilisation non prévu lors de l'évaluation préliminaire.
\par\bigskip

Une convention de nommage a été établie pour faciliter la gestion et la maintenance des GPOs, 
...
Tous les tests de conformités sont réalisés à l'aide du module SCA de Wazuh. Nous verrons ici l'évolution de la conformité au benchmark CIS du poste "HP026"
...
\\
Pour actualiser les résultats du module SCA sur ce poste, nous pouvons soit redémarrer le poste soit utiliser la commande\\
\begin{figure}[H]
  \centering
...
Nous observons que la conformité est faible, de seulement 32\%. La conformité moyenne des postes de travail avant le déploiement de ces GPOs se situait entre 25 et 35\%.\\
\\
...
Au cours de notre utilisation du module SCA de Wazuh, nous nous sommes aperçus de certaines limitations :\\
\\
...
\\
Cependant, nous avons constaté après l'application de plusieurs GPO que le taux de conformité n'augmentait pas malgré le fait que les GPO soient appliquées sur les machines. Nous avons donc vérifié en générant un rapport des GPOs appliquées sur le poste que les GPOs étaient bel et bien actives :
\begin{figure}[H]
  \centering
...
\label{fig:test-sca}
\caption{Test manuel d'une commande utilisée par l'agent, sur le poste HP026}
\end{figure}
\newpage
En ouvrant le fichier \textit{YAML} envoyé par Wazuh sur les machines nous avons constaté que quand le module SCA testait 
l'application d'une GPO, il vérifiait la configuration d'une clé de registre spécifique. Pour les GPO qui n'étaient pas liées à des clés de registre, le module teste directement la conformité d'un paramètre via une commande PowerShell.\\
...
Nous avons donc été obligés de modifier tout ce fichier pour traduire chaque instance de test de GPO utilisant la commande \textbf{"auditpol"}.\\
\\
Ce changement dans le fichier de test a engendré un autre problème. En effet, l'agent Wazuh exécute les commandes via PowerShell, ce qui limite le support des caractères UTF-8.
...
Nous pouvons voir ici un exemple de commande de test de ces GPO mise à jour :
...
Il ne suffit donc maintenant plus qu'à déployer automatiquement le nouveau fichier de test sur le parc informatique. Nous pouvons simplement utiliser une GPO pour modifier ce fichier sur les postes. Ce fichier sera donc disponible dans le volume partagé \textbf{SYSVOL}.\\
\begin{figure}[H]
  \centering
...
L'utilisation de la voiture a donc un impact certain mais qui reste limité car peu fréquent. Ces estimations ont été faite avec le simulateur en ligne de l'ADEME.
\par\bigskip
...
Au cours de ce stage chez Mesulog, j'ai pu rencontrer des sujets variés et formateurs, comme le traitement et l'analyse de données, la mise en place d'un système de supervision et de sécurisation du réseau informatique d'une entreprise.\\
\\
L'installation et la configuration de Wazuh en tant que solution SIEM a permis d'établir un système complet de collecte et d'analyse des événements réseau, avec la création de décodeurs, de règles et d'alertes personnalisées ; ce qui m'a permis d'explorer en profondeur les possibilités de Wazuh. Notamment avec l'intégration d'un script externe à l'API pour l'analyse des évenements du pare-feu Sophos.\\
\\
...
\\
Ce stage m'a aussi permis de développer des compétences professionnelles importantes comme la maîtrise des solutions SIEM, une compréhension approfondie de la gestion de Windows Server et d'Active Directory, la capacité à identifier des problèmes et la capacité à utiliser des moyens alternatifs (installation de Suricata dans un conteneur, externalisation de la mise à jour des règles d'IP malicieuses) pour assurer le bon fonctionnement des services.\\
\\
Plusieurs pistes d'amélioration existent cependant. De manière évidente une poursuite des itérations de mise en conformité avec le benchmark CIS Windows 11, mais aussi avec l'ajout de nombreuses autres règles pour Wazuh afin d'éliminer les dernières zones d'ombre du parc informatique de Mesulog.\\
\\
Dernièrement, ce stage m'a confirmé mon orientation vers le domaine de la cybersécurité et de l'administration systèmes \& réseaux. J'ai acquis une expérience concrète qu'il me tarde de réutiliser dans mes emplois futurs.\\
\\
...
\bibitem{[18]} \textbf{Utilisation de l'IA : l'IA n'a pas été utilisée pour produire du texte, mais a été utilisée pour corriger les fautes d'orthographe et assister à la création de certains schémas.}
