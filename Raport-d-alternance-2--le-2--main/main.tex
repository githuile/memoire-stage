...
\subsection{Base de données Access}
  ...
  Il était donc courant d'avoir des enregisrtements partiels, erronés ou avec un mauvais format. L'objectif final était de produire différents graphiques pour analyser l'évolution de la production de modules, appelés "V1" ou "V2".\\

  Voici donc la liste des indicateurs (KPI) à extraire de ces données :

  \begin{enumerate}
    \item[$\bullet$] Depuis 2020, par banc et par type de module : 
      \begin{enumerate}
        \item[$\bullet$] Le nombre de modules testés au total
        \item[] Le nombre de modules testés en Production
        \item[] Le nombre de modules testés en production validés
        \item[] Le nombre de modules testés en production non-validés
        \item[] Le ratio de modules V2 sur l'ensemble
        \item[] L'Effort Pass* moyen
        \item[] L'Effort Pass* médian
        \item[] Le Time to First Pass(TtFP)* en heure
        \item[] Le nombre total d'essais en Production
        \item[] Le ratio d'essais validés
        \item[] Le ratio d'essais non-validés
        \item[] Le ratio d'essais coupés avant la fin
        \item[] Le ratio d'essais en erreur
        \item[] Le nombre d'essais en erreur les Lundis matin
        \item[] Le ratio
        \item[] Le nombre d'essai en erreur les autres jours le matin
        \item[] le
      \end{enumerate}
    \item[$\bullet$]  
  \end{enumerate}

  12 setup puis 18 extraction
