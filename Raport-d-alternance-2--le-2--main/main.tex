...
\begin{center} Suivi des missions d'administration Sécurité, Systèmes et Réseaux au sein d'une PME\\
\vspace*{0.5cm}
MESULOG SAS\\
\vspace*{1.5cm}
...
Premièrement, je tiens à préciser que cette année d'alternace au sein de Mesulog fût une grande opportunité m’offrant une seconde expérience au sein du monde professionnel. Je considère avoir beaucoup appris sur les environnements professionnel, la sécurité, le réseau ainsi que l’administration système grâce aux
...
\begin{itemize}
\item[$\bullet$] \textbf{Luc Desruelle (RSSI - Mesulog)} mon tuteur d'alternance qui m'a aisé à contextualiser les projets sur lesquels je travaillais
\item[] 

\item[$\bullet$] \textbf{Benjamin Lerch} mon deuxième tuteur, référent de la partie technique, il m'a notamment aidé à mieux connaître l'architecture du SI de l'entreprise.
\item[] 
...
Dans le cadre de mon Bachelor Universitaire Technologique en informatique à l'IUT2 de Grenoble, j'ai eu l'opportunité d'effectuer une année d'alternance au sein
...
Cette alternance s'inscrit dans le cadre de la sécurisation su système d'information de l'entreprise, un enjeu majeur afin de garantir la confidentialité,
 l'intégrité et la disponibilité des données et des services. Les missions qui m'ont été confiées ont couvert différents aspects de l'administration système,
  de la cybersécurité et du développement d'outils internes, avec pour objectif d'améliorer la résilience du système d'information face aux menaces actuelles.

\par\bigskip
 Afin de mieux comprendre les missions réalisées au cours de cette année, il est essentiel de présenter le cadre professionnel dans lequel 
elles s’inscrivent. Cette première partie introduit le contexte institutionnel et organisationnel de l’organisme d’accueil de mon alternance.
...
À l’origine, Mesulog et Nerys constituaient deux entreprises distinctes, ayant respectivement pour domaine d'activités le développement de logiciels de
 test et de mesures pour l'une et la conception de bancs de test, baies et coffrets électriques pour les environnements industriels pour l'autre. Leur 
 rapprochement a conduit à la structuration de \textbf{Nerys Group}, société mère intégrant Nerys et Mesulog.
 \par\bigskip
 Cette mutualisation permet ainsi le regroupement des expertises techniques dans les domaines du test, de la mesure et de 
 l’ingénierie logicielle associée aux bancs d’essais. Ce rapprochement intégre également la société Territoire de Changements devenue ensuite Nerys Group 
...
% --- Ligne 1 : Président + Responsable administrative ---
\node[boxdark] (president) {Luc Desruelle\\Directeur};
\node[boxdark, right=1.8cm of president] (admin) {Emilie Evrard\\Responsable administrative};

% --- Ligne 2 : Chefs de projet ---
\node[boxmed, below left=1.6cm and 2.8cm of president] (lamy) {Damien Lamy\\Chef de projet};
\node[boxmed, below=1.6cm of president] (andrieu) {Adrien Andrieu\\Chef de projet};

% --- Ligne 3 : Développeurs ---
\node[boxlight, below left=1.6cm and 2.4cm of andrieu] (lerch) {Benjamin Lerch\\Développeur ingénieur};
\node[boxlight, below=1.6cm of andrieu] (asaro) {Anthony Asaro\\Développeur technicien};
\node[boxlight, below right=1.6cm and 2.4cm of andrieu] (bertin) {Thomas Bertin\\Développeur ingénieur};

% --- Ligne 4 : Stagiaires / Alternant ---
\node[boxlight, below left=1.6cm and 2.4cm of asaro] (sechi) {Kenzo Sechi\\Alternant};
\node[boxgreen, below=1.6cm of asaro] (kilian) {Verger Kilian\\Stagiaire};
\node[boxlight, below right=1.6cm and 2.4cm of asaro] (miquel) {Vianney Miquel\\Stagiaire};

% --- Connexions ---
% Président <-> Admin
\draw[line] (president.east) -- (admin.west);

% Président -> chefs de projet
\draw[line] (president.south) -- ++(0,-0.5) -| (lamy.north);
\draw[line] (president.south) -- ++(0,-0.5) -- (andrieu.north);

% Andrieu -> développeurs
\draw[line] (andrieu.south) -- ++(0,-0.5) -| (lerch.north);
\draw[line] (andrieu.south) -- ++(0,-0.5) -- (asaro.north);
\draw[line] (andrieu.south) -- ++(0,-0.5) -| (bertin.north);

% Asaro -> niveau 4
\draw[line] (asaro.south) -- ++(0,-0.5) -| (sechi.north);
\draw[line] (asaro.south) -- ++(0,-0.5) -- (kilian.north);
\draw[line] (asaro.south) -- ++(0,-0.5) -| (miquel.north);
...
\section{Imapct environnemental et sociétal}
...
\subsection{Valeurs et axes structurants}
...
% --- Ligne 1 : Président + Responsable administrative ---
\node[boxdark] (president) {Luc Desruelle\\Président};
\node[boxdark, right=1.8cm of president] (admin) {Emilie Evrard\\Responsable administrative};
...
L'organigramme présenté ci-dessus permet de visualiser la structuration actuelle de Mesulog et de comprendre la répartition des rôles au sein de l'équipe.
 Il illustre une organisation construite progressivement, au fil de l'évolution de l'activité.
...
\par\bigskip
 En juillet 2020, l'entreprise évolue en SAS et intègre Nerys Group. Cette étape marque un ajustement de la gouvernance, avec une direction 
...
techniques ont conduit l’entreprise à structurer et renforcer ses pratiques en matière de cybersécurité et de gouvernance du SI. Dans ce cadre, les missions qui m’ont
 été confiées répondent à plusieurs enjeux transverses : assurer la conformité aux exigences réglementaires et normatives, améliorer la résilience du système 
...
bien que certaines missions comme \textit{l'homologation du SI classé "DR"} ou bien la \textit{migration de l'AD} étaient prévus pour une date donnée ou une date butoire,
